...
for przypadek in przypadki: 
    ...
    przypadek = przypadek.drop(przypadek[przypadek['j'] == przypadek['j'].max()].index, inplace = True) # drop last row from every time step
...
for przypadek in przypadki:
    multiplier = int(round(jmax/przypadek['j'].max(),0))
    przypadek['j'] = multiplier * przypadek['j']
...
#jmax = case1['j'].max()
# w przypadku roznej ilosci komorek, wybierz najwieksza
